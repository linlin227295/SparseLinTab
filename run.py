...
from sparselintab.column_encoding_dataset import ColumnEncodingDataset
from sparselintab.configs import build_parser
from sparselintab.distribution import distributed_train_wrapper
from sparselintab.train import Trainer
from sparselintab.utils.model_init_utils import init_model_opt_scaler_from_dataset
from sparselintab.utils.viz_att_maps import viz_att_maps
...
def run_cv_splits(wandb_args, args, c, wandb_run):
    dataset = ColumnEncodingDataset(c)  # 加载数据集

    #######################################################################
    # Distributed Setting   分布式设置
    if c.mp_distributed:
    ...
    for cv_index in range(starting_cv_index, total_n_cv_splits):
        ...
        # New wandb logger for each run  Wandb初始化，为每个交叉验证的分割生成一个新的wandb运行
        if cv_index > 0:
        ...
        trainer.train_and_eval()  # 启动模型训练，并在每个epoch后进行评估  step1

        wandb_run.finish()  # 结束当前Wandb运行
