...
from sparselintab.utils.model_init_utils import (
...
class EarlyStopCounter:
    def __init__(self, c, data_cache_prefix, metadata, wandb_run, cv_index,
        ...
        # 用于记录验证损失未改进的连续 epoch 数。若这个值超过 patience（耐心值），则触发早停，即停止训练。
        ...
        # self.min_val_loss 用于记录到目前为止验证集上观察到的最小损失。
        self.min_val_loss = float('inf')

        self.patience = c.exp_patience  # self.patience 是用于早停的耐心值，即验证损失连续多少个 epoch 没有改善时触发早停。
        self.c = c
