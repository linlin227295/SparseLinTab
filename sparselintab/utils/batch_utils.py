...
def collate_with_pre_batching(batch):
    ...
    We avoid adding a batch dimension, as for sparselintab we have pre-batched data,
    ...
    if isinstance(elem, container_abcs.Mapping):
        return elem  # Just return the dict, as there will only be one in sparselintab
