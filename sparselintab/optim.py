...
class TradeoffAnnealer:
    def __init__(self, c, num_steps=None):
        ...
        self.step = self.step_map[self.name]  # 设置当前的调度步骤

        if num_steps > 0:  # 快进到指定步骤
...
class LRScheduler:
    ...
    def construct_auto_scheduler(self):
        ...
        elif self.name == 'cosine_cyclic':  # 余弦循环
            # 配置 CosineLRScheduleConfig 所需的参数
            cosine_cfg = cosine_lr_scheduler.CosineLRScheduleConfig(
                warmup_updates=num_warmup_steps,  # warmup 阶段的步数
                warmup_init_lr=1e-7,  # 初始学习率
                lr=self.c.exp_lr,  # 最大学习率
                t_mult=2.0,  # 周期倍增因子
                lr_period_updates=num_warmup_steps * 2,  # 每个周期的步数
                lr_shrink=0.5  # 学习率缩减因子
            )
            optim = FairseqOptimizer(None)
            optim._optimizer = optim.optimizer = self.optimizer
            self.scheduler = cosine_lr_scheduler.CosineLRSchedule(cfg=cosine_cfg, fairseq_optimizer=optim)
