...
import numpy as np
import torch
from sparselintab.datasets.Fault_prediction import FaultPredictionDataSet
from sparselintab.batch_dataset import sparselintabBatchDataset
from sparselintab.datasets.base import BaseDataset
from sparselintab.datasets.boston_housing import BostonHousingDataset
from sparselintab.datasets.breast_cancer import (
    BreastCancerClassificationDataset, BreastCancerDebugClassificationDataset)
from sparselintab.datasets.cifar10 import CIFAR10Dataset
from sparselintab.datasets.concrete import ConcreteDataset
from sparselintab.datasets.debug import DebugDataset
from sparselintab.datasets.forest_cover import ForestCoverClassificationDataset
from sparselintab.datasets.higgs import HiggsClassificationDataset
from sparselintab.datasets.image_utils import load_image_dataloaders
from sparselintab.datasets.income import IncomeDataset
from sparselintab.datasets.kick import KickDataset
from sparselintab.datasets.mnist import MNISTDataset
from sparselintab.datasets.poker_hand import PokerHandDataset
from sparselintab.datasets.protein import ProteinDataset
from sparselintab.datasets.yacht import YachtDataset
from sparselintab.utils.cv_utils import (
    get_class_reg_train_val_test_splits, get_n_cv_splits)
from sparselintab.utils.encode_utils import encode_data_dict
from sparselintab.utils.memory_utils import get_size
from sparselintab.utils.preprocess_utils import (
    get_matrix_from_rows)

DATASET_NAME_TO_DATASET_MAP = {
    'fault-prediction': FaultPredictionDataSet,
...
class ColumnEncodingDataset:
    ...
    batching strategy for sparselintab model.
    """
    ...
    def __init__(self, c, device=None):
        ...
        self.mode = None  # 当前的数据集模式，可以是train、val、test
        self.valid_modes = ['train', 'val', 'test']  # 一个包含合法模式的列表

        # Retrieve dataset class and metadata  加载数据集类和元数据
        try:
            self._dataset = DATASET_NAME_TO_DATASET_MAP[  # 实例化相应的数据集类
                self.c.data_set](self.c)  # type: BaseDataset
        except KeyError:
            raise NotImplementedError(
                f'Have not implemented dataset {self.c.data_set}')

        # Retrieve pathing information  初始化缓存路径和交叉验证分割
    ...
    def reset_cv_splits(self):
        self.dataset_gen = self.run_preprocessing_and_caching()  # 数据集生成器
    ...
    def get_model_details(self, c):
        ...
        return c.model_class == 'sparselintab'
    ...
    def load_torch_dataset(self, data_dict):
        ...
        return sparselintabBatchDataset(
    ...
    def run_preprocessing_and_caching(self):
        ...
        if not self.c.data_force_reload:  # 缓存机制
        ...
        # Skip dataset generation and caching if files are available    检查是否需要重新生成数据集
...
class sparselintabDataset(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, dataset: ColumnEncodingDataset):
        super(sparselintabDataset).__init__()
