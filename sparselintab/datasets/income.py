...
class IncomeDataset(BaseDataset):
    ...
    def load(self):
        ...
        # If the data is a pandas DataFrame, convert to numpy array
        if isinstance(self.data_table, pd.DataFrame):
            self.data_table = self.data_table.to_numpy()

        # If it's an ndarray, we can directly work with it
        if isinstance(self.data_table, np.ndarray):
            # Delete last few rows (e.g., remove last 5 rows)
            if self.c.exp_batch_size != -1:
                self.N = self.data_table.shape[0]
                batch_size = self.c.exp_batch_size
                rem = self.N % batch_size

                self.data_table = self.data_table[:-rem, :]  # Remove the last 'n' rows
