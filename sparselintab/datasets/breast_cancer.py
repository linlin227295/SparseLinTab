...
from sparselintab.utils.data_loading_utils import download


class BreastCancerClassificationDataset(BaseDataset):
    def __init__(self, c):
        super(BreastCancerClassificationDataset, self).__init__(
            fixed_test_set_index=None)  # fixed_test_set_index=None表示不使用固定的测试集索引
        self.c = c
    ...
    def load(self):
        ...
        # For breast cancer, target index is the first column  设置目标列
...
def load_and_preprocess_breast_cancer_dataset(c):
    ...
    if c.exp_smoke_test:
        ...
        # Speculate some spurious missing features  推测虚假的缺失特征
