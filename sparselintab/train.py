...
from sparselintab.column_encoding_dataset import ColumnEncodingDataset, sparselintabDataset
from sparselintab.loss import Loss
from sparselintab.optim import LRScheduler
from sparselintab.optim import TradeoffAnnealer
from sparselintab.utils import debug
from sparselintab.utils.batch_utils import collate_with_pre_batching
from sparselintab.utils.encode_utils import torch_cast_to_dtype
from sparselintab.utils.eval_checkpoint_utils import EarlyStopCounter, EarlyStopSignal
from sparselintab.utils.logging_utils import Logger
...
class Trainer:
    def __init__(
            ...
            torch_dataset: sparselintabDataset = None,
        ...
        self.early_stop_counter = EarlyStopCounter(  # 早停计数器
        ...
        # Initialize from checkpoint, if available   从检查点恢复模型
        num_steps = 0
        ...
        # recorded in checkpoint.   Tradeoff Annealer权衡标签损失与数据增强损失之间的权重
    ...
    def get_max_epochs(self):
        ...
        return int(
            np.ceil(self.c.exp_num_total_steps / num_steps_per_epoch))  # np.ceil向上取整
    ...
    def per_epoch_train_eval(self, epoch):
        ...
        end_experiment = end_experiment or (  # 条件判断：是否在调试模式下立即结束实验
                self.c.debug_eval_row_interactions and epoch == 2)

        eval_model = end_experiment or self.eval_check(epoch)  # 检查是否需要进行评估
        ...
        if self.c.debug_eval_row_interactions:
            ...
            train_loss = self.run_epoch(dataset_mode='train', epoch=epoch,  # step3
                                        eval_model=False)
    ...
    def train_and_eval(self):
        ...
        self.logger.start_counting()  # 启动了日志记录器的计时功能

        if self.is_distributed and self.c.mp_no_sync != -1:  # 分布式训练与同步控制
            curr_epoch = 1
            ...
            for epoch in range(1, self.max_epochs + 1):  # 非分布式训练流程
                if self.per_epoch_train_eval(epoch=epoch):  # 每个训练周期执行该函数，若该函数返回 True，则退出训练  step 2
                    break
    ...
    def eval_model(self, train_loss, epoch, end_experiment):
        ...
        if not (self.c.debug_eval_row_interactions and epoch == 2):
            # Early stopping check早停检查 -- TODO: consider loss other than label?
        ...
        if not self.c.debug_eval_row_interactions:
            if (counter == EarlyStopSignal.STOP) or end_experiment:  # 加载最佳模型（如果早停或达到最大训练步数）
        ...
        # Check if we need to eval test 测试集评估
    ...
    def run_epoch(self, dataset_mode, epoch, eval_model=False):
        ...
        # Model prep
        # We also want to eval train loss 模型准备：训练或评估模式
        ...
        self.dataset.set_mode(mode=dataset_mode, epoch=epoch)  # 设置数据集模式
        ...
        if self.is_distributed:
            ...
            batch_sizes = batch_dataset.batch_sizes
            print(batch_sizes)
            extra_args = {}
            ...
            batch_iter = torch.utils.data.DataLoader(
                ...
                **extra_args)
            batch_iter = tqdm(
                batch_iter, desc='Batch') if self.c.verbose else batch_iter  # 如果配置 verbose 为 True，使用 tqdm 进度条包来包装数据加载器，显示当前加载进度。

        if (eval_model and self.c.debug_eval_row_interactions  # 行交互调试模式
                and epoch == 2 and dataset_mode in {'test'}):
            if self.c.debug_eval_row_interactions_timer is not None:
                self.set_row_corruption_timer()

        for batch_index, batch_dict_ in enumerate(batch_iter):  # 批量处理
            if self.c.debug_row_interactions:  # 如果启用了行交互调试（debug_row_interactions），会对批次数据进行修改，例如在训练过程中随机改变某些行的数据。
        ...
        if eval_model or batch_GD:
            ...
            loss_dict = self.loss.finalize_epoch_losses(eval_model)  # 损失字典更新和反向传播
    ...
    def run_batch(self, batch_dict, dataset_mode, eval_model,
        ...
        # label_mask_matrix. Else, it is just None. 从 batch_dict 获取数据
        ...
        if self.c.debug_label_leakage:  # 调试标签泄露检查
            debug.leakage(
                self.c, batch_dict, masked_tensors, label_mask_matrix,
                dataset_mode)

        # Construct ground truth tensors 构建真实标签张量
        ...
        if not self.c.data_set_on_cuda:
            ...
            # Cast tensors to appropriate data type  数据迁移到指定设备（CUDA/GPU）
        ...
        with torch.cuda.amp.autocast(enabled=self.c.model_amp):
            self.forward_and_loss(**forward_kwargs)  # 执行前向传播，并计算模型的损失。
        ...
        if (dataset_mode == 'train' and self.c.exp_minibatch_sgd
            ...
            loss_dict = self.loss.finalize_batch_losses()  # 当前批次的损失值。
            train_loss = loss_dict['total_loss']
            ...
            self.scaler.scale(train_loss).backward()  # 使用 self.scaler.scale(train_loss).backward() 来执行反向传播，计算梯度。使用混合精度时，scaler 会自动调整损失值的尺度。
            ...
            self.scaler.step(self.optimizer)  # 调用 scaler.step() 更新优化器步骤

            # Updates the scale for next iteration.
            self.scaler.update()  # 调用 scaler.update() 更新 AMP 的缩放因子。

            if self.tradeoff_annealer is not None:
                self.tradeoff_annealer.step()  # 权衡退火更新

            self.scheduler.step()  # 调用 self.scheduler.step() 更新学习率。
            self.optimizer.zero_grad()  # 清除梯度

            if print_n and (self.scheduler.num_steps % print_n == 0):  # 打印日志
                self.logger.intermediate_log(
                    loss_dict=loss_dict,
                    num_steps=self.scheduler.num_steps,
                    batch_index=batch_index, epoch=epoch)

        # Update the epoch loss info with detached minibatch losses 更新损失信息
    ...
    def forward_and_loss(
        ...
        extra_args = {}
        print('masked_tensors:', masked_tensors)
        if eval_model:
            with torch.no_grad():
