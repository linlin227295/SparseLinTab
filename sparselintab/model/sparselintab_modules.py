...
class LinearAttentionHead(nn.Module):
    ...
    def __init__(
            self, dim_Q, dim_KV, dim_emb, dim_out, E_proj, dim_k, dim_in_proj, c):
        """
    ...
    def forward(self, X, Y):
        ...
        Q_ = torch.cat(Q.split(self.dim_split, 2), 0)  # 8N*D*4
        K_ = torch.cat(K.split(self.dim_split, 2), 0)
        V_ = torch.cat(V.split(self.dim_split, 2), 0)

        # LinFormer
        K_ = K_.transpose(1, 2)  # 8N*4*D
        if self.is_proj_tensor:
            self.E = self.E.to(K_.device)
            K_ = torch.matmul(K_, self.E)
        else:
            K_ = self.E(K_)  # 8N*4*dim_k
        A = torch.matmul(Q_, K_)  # 8N*D*dim_k
        # TODO: track issue at
        ...
        V_ = V_.transpose(1, 2)  # 8N*4*D
        if self.parameter_sharing == 'headwise':
            F_proj = get_EF(self.dim_in_proj, self.dim_k, self.method, self.dim_split)
            if self.is_proj_tensor:
                F_proj = F_proj.to(V_.device)
                V_ = torch.matmul(V_, F_proj)
            else:
                V_ = F_proj(V_)
        else:
            if self.is_proj_tensor:
                self.E = self.E.to(V_.device)
                V_ = torch.matmul(V_, self.E)  # 8N*4*dim_k
            else:
                V_ = self.E(V_)
        V_ = V_.transpose(1, 2)  # 8N*dim_k*4

        multihead = A.bmm(V_)  # 8N*D*4
...
class MHSA(nn.Module):
    ...
    def __init__(self, dim_in, dim_emb, dim_out, E_proj, dim_k, dim_in_proj, c):
        super(MHSA, self).__init__()
        self.Linear = LinearAttentionHead(dim_in, dim_in, dim_emb, dim_out, E_proj, dim_k, dim_in_proj, c)
